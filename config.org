...
When using ~org-edit-special~ to edit a src block the function ~evil-write~ should return to the main buffer saving changes to region, whilst ~evil-quit~ should abort:
#+begin_src emacs-lisp :tangle emacs.d/init.el
